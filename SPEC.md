...
Предпочтительно использовать актуальные универсальные методы Bitrix:

- `crm.item.list`, `entityTypeId = 2` — сделки;
- `crm.item.list`, `entityTypeId = 3` — контакты;
- `crm.deal.contact.items.get` — все контакты, связанные со сделкой;
...
| `closed_at` | дата и время завершения | обязательно для закрытой сделки |
...
| Цикл сделки | `closed_at - created_at` |
...
- выручка относится к периоду по `closed_at`;
...
- финансовые отчёты фильтруются по `closed_at`;
